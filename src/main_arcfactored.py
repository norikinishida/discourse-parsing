...
def main(args):
    ...
    base_dir = "arcfactored.%s" % config_name
    ...
    path_train_losses = os.path.join(config["results"], base_dir, prefix + ".training.losses.jsonl")
        ...
        for instance_i in range(0, n_train, batch_size):
            ...
            for data in train_dataset[perm[instance_i:instance_i + batch_size]]:
                ...
                one_loss_attachment, \
                    ...
                    one_loss_relation, \
                    one_acc_relation, \
                    n_arcs = parser.compute_loss(data=data)
