...
def main():
    ...
    for split in ["train", "dev", "test"]:
        ...
        path_output = os.path.join(config["caches-dep"], "molweni.%s.bert-base-cased.npy" % split)
        np.save(path_output, dataset)

        # Cache gold arcs
        path_output = os.path.join(config["caches-dep"], "molweni.%s.gold.arcs" % split)
    ...
    utils.write_vocab(os.path.join(config["caches-dep"], "molweni.relations.vocab.txt"),
                      relations,
...
def preprocess(tokenizer_wrapper, split):
    ...
    path_root = os.path.join(config["data"], "molweni-compiled", split)
