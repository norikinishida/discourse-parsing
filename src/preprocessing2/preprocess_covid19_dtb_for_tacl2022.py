...
def main():
    ...
    for split in ["dev", "test"]:
        ...
        path_output = os.path.join(config["caches-tacl2022"], "mapped-covid19-dtb.%s.scibert_scivocab_uncased.npy" % split)
        # path_output = os.path.join(config["caches-tacl2022"], "mapped-covid19-dtb.%s.BiomedNLP-PubMedBERT-base-uncased-abstract.npy" % split)
        np.save(path_output, dataset)

        # Cache gold arcs
        path_output = os.path.join(config["caches-tacl2022"], "mapped-covid19-dtb.%s.gold.arcs" % split)
    ...
    utils.write_vocab(os.path.join(config["caches-tacl2022"], "mapped-covid19-dtb.relations.vocab.txt"),
                     relations,
