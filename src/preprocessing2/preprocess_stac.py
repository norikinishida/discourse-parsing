...
def main():
    ...
    for split in ["train", "dev", "test"]:
        ...
        path_output = os.path.join(config["caches-dep"], "stac.%s.bert-base-cased.npy" % split)
        np.save(path_output, dataset)

        # Cache gold labels
        path_output = os.path.join(config["caches-dep"], "stac.%s.gold.arcs" % split)
    ...
    utils.write_vocab(os.path.join(config["caches-dep"], "stac.relations.vocab.txt"),
                     relations,
