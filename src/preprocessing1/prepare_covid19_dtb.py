...
def main(args):
    ...
    filenames = [n for n in filenames if n.endswith(".dep")]
    filenames.sort()

    skip_count = 0
    for filename in pyprind.prog_bar(filenames):
        edus, sents, sentence_boundaries, paragraph_boundaries, disc_arcs = read_data(os.path.join(path_in, filename))
        ...
        with open(os.path.join(path_out, filename.replace(".dep", ".edus.tokens")), "w") as f:
            for edu in edus:
                edu = " ".join(edu)
                f.write("%s\n" % edu)

        with open(os.path.join(path_out, filename.replace(".dep", ".sentence_boundaries")), "w") as f:
            for begin_i, end_i in sentence_boundaries:
                f.write("%d %d\n" % (begin_i, end_i))

        with open(os.path.join(path_out, filename.replace(".dep", ".paragraph_boundaries")), "w") as f:
            for begin_i, end_i in paragraph_boundaries:
                f.write("%d %d\n" % (begin_i, end_i))

        with open(os.path.join(path_out, filename.replace(".dep", ".arcs")), "w") as f:
        ...
        with open(os.path.join(path_out, filename.replace(".dep", ".edus.postags")), "w") as fp,\
             open(os.path.join(path_out, filename.replace(".dep", ".edus.arcs")), "w") as fa:
            begin_tok_i = 0
            for edu in edus:
                length = len(edu)
...
def read_data(path):
    ...
    for end_edu_i, x in enumerate(data):
        ...
        if x["is_send_end"]:
            ...
            end_sent_i += 1
            if x["is_para_end"]:
                paragraph_boundaries.append((begin_sent_i, end_sent_i))
                begin_sent_i = end_sent_i + 1
    ...
    return edus, sents, sentence_boundaries, paragraph_boundaries, disc_arcs
